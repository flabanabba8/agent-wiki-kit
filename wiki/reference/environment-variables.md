...
Variables control model selection, authentication, routing, limits and feature toggles; most also have a settings key ([[settings]]), a CLI flag ([[cli-reference]]) or an in-session command. The official list runs to several hundred rows, so rarely used display-name, region and cache-tuning variants are summarized by pattern.

## Setting and precedence

- **Shell:** `export API_TIMEOUT_MS="1200000"` before `claude`; shell values are read at startup.
...
- A settings `env` value beats the shell; between files normal settings precedence applies, so managed settings win.
...
| `ANTHROPIC_PROFILE` | Anthropic profile to authenticate with |
| `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_WORKSPACE_ID` | Workload Identity Federation |
...
| `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY` | Fill `/model` from the gateway's `/v1/models` |
...
| `CLAUDE_CODE_GATEWAY_HINT_HEADERS`, `CLAUDE_GATEWAY_DRAIN_TIMEOUT_MS` | `1` sends the gateway routing-hint request headers; Claude apps gateway drain window on `SIGTERM` |

Setup walkthroughs: [[cloud-providers]] and [[llm-gateways]].
...
| `API_TIMEOUT_MS` | Per-request timeout, default 600000 (10 min) |
| `CLAUDE_STREAM_IDLE_TIMEOUT_MS`, `CLAUDE_BYTE_STREAM_IDLE_TIMEOUT_MS`, `CLAUDE_STREAM_FIRST_BYTE_TIMEOUT_MS`, `API_FORCE_IDLE_TIMEOUT` | Streaming stall timers; explicit `CLAUDE_STREAM_IDLE_TIMEOUT_MS` minimum is 300000 |
...
| `CLAUDE_CODE_RETRY_WATCHDOG` | Unattended runs: retry `429`/`529` capacity errors indefinitely, and up to 300 retries for other transient errors |
...
| `CLAUDE_CODE_MAX_OUTPUT_TOKENS` | Max output tokens; raising it shrinks usable context before compaction |
| `CLAUDE_CODE_DISABLE_1M_CONTEXT` | Hold sessions to a 200K window |
...
| `CLAUDE_CODE_DISABLE_ADVISOR_TOOL` | Disable the advisor tool; `--advisor` is accepted but ignored |
| `FALLBACK_FOR_ALL_PRIMARY_MODELS` | Stop retrying repeated overloads on every model when no fallback is configured |
...
| Variable | Purpose |
|---|---|
| `BASH_DEFAULT_TIMEOUT_MS`, `BASH_MAX_TIMEOUT_MS`, `BASH_MAX_OUTPUT_LENGTH` | Bash timeout default 120000, ceiling 600000; output read back into a result, default 30000, max 150000 |
...
| `CLAUDE_CODE_USE_POWERSHELL_TOOL`, `CLAUDE_CODE_POWERSHELL_RESPECT_EXECUTION_POLICY`, `CLAUDE_CODE_GIT_BASH_PATH` | PowerShell tool and Windows shell setup |
...
| `CLAUDE_CODE_ENABLE_TODO_TOOLS`, `CLAUDE_CODE_ENABLE_TASKS`, `CLAUDE_CODE_TASK_LIST_ID`, `TASK_MAX_OUTPUT_LENGTH` | Task-tracking tools on every model; `0` selects `TodoWrite`; shared task list; background output size |
| `CLAUDE_CODE_MAX_TOOL_USE_CONCURRENCY` | Parallel read-only tools and subagents, default 10 |
| `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB`, `CLAUDE_CODE_SCRIPT_CAPS` | Strip credentials from subprocesses; per-script call caps |
...
| `CLAUDE_CODE_FORK_SUBAGENT` | Fork mode: `1` also in `-p` and SDK, `0` off everywhere |
| `CLAUDE_CODE_DISABLE_EXPLORE_PLAN_AGENTS`, `CLAUDE_AGENT_SDK_DISABLE_BUILTIN_AGENTS` | Remove built-in subagents |
| `CLAUDE_AUTO_BACKGROUND_TASKS`, `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS`, `CLAUDE_ASYNC_AGENT_STALL_TIMEOUT_MS` | Auto-backgrounding on/off; subagent stall timeout |
| `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` | Enable agent teams |
| `CLAUDE_CODE_DISABLE_WORKFLOWS`, `CLAUDE_CODE_DISABLE_AGENT_VIEW`, `CLAUDE_CODE_DISABLE_CRON` | Turn off workflows, agent view and background agents, scheduled tasks |
...
| `CLAUDE_CODE_AUTO_MODE_SERVER` | `1` picks the platform's server-side auto-mode classifier ([[permissions-and-modes]]) |
| `CLAUDE_CODE_GOAL_CHECKIN_MINUTES`, `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP` | `/goal` check-in interval (30); consecutive Stop-hook blocks (8) |
| `CLAUDE_CODE_MAX_TURNS`, `MAX_STRUCTURED_OUTPUT_RETRIES`, `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS` | Turn cap; `--json-schema` attempts (5); `-p` wait for background tasks |
| `CLAUDE_CODE_RESUME_INTERRUPTED_TURN`, `CLAUDE_CODE_RESUME_PROMPT`, `CLAUDE_CODE_EXIT_AFTER_STOP_DELAY` | SDK resume and exit behavior |
| `CLAUDE_CODE_SKIP_PROMPT_HISTORY`, `CLAUDE_CODE_SIMPLE`, `CLAUDE_CODE_SAFE_MODE`, `CLAUDE_CODE_RESTRICTED` | No transcripts; bare, safe and restricted modes |

## MCP, plugins, skills

| Variable | Purpose |
|---|---|
| `MCP_TIMEOUT`, `MCP_TOOL_TIMEOUT`, `CLAUDE_CODE_MCP_TOOL_IDLE_TIMEOUT` | Server startup (30000), tool execution, idle abort |
| `MAX_MCP_OUTPUT_TOKENS` | Tool response cap, default 25000 |
| `ENABLE_TOOL_SEARCH` | Deferral: unset defers all MCP tools; `auto`, `auto:N`, `true`, `false` |
| `MCP_CONNECTION_NONBLOCKING`, `MCP_CONNECT_TIMEOUT_MS`, `MCP_SERVER_CONNECTION_BATCH_SIZE`, `MCP_REMOTE_SERVER_CONNECTION_BATCH_SIZE` | Startup connection behavior |
...
| `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` | How long a non-interactive first turn waits for connecting servers |
...
| `CLAUDE_CODE_DISABLE_BUNDLED_SKILLS`, `CLAUDE_CODE_DISABLE_POLICY_SKILLS`, `CLAUDE_CODE_SYNC_SKILLS`, `SLASH_COMMAND_TOOL_CHAR_BUDGET` | Bundled and managed skills; claude.ai skill sync in `-p`; skill listing budget |
...
| `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` | Block auto-updates, telemetry, error reporting, `/feedback`, release notes and availability checks |
...
| `CLAUDE_CODE_ENABLE_TELEMETRY` | Turn on OpenTelemetry export |
| `OTEL_LOG_USER_PROMPTS`, `OTEL_LOG_ASSISTANT_RESPONSES`, `OTEL_LOG_TOOL_DETAILS`, `OTEL_LOG_TOOL_CONTENT`, `OTEL_LOG_RAW_API_BODIES` | Opt content into telemetry (redacted by default) |
| `OTEL_LOG_MANAGED_SETTINGS` | Add redacted managed settings and their digests to the managed-settings OTel event |
| `OTEL_METRICS_INCLUDE_SESSION_ID`, `OTEL_METRICS_INCLUDE_ACCOUNT_UUID`, `OTEL_METRICS_INCLUDE_VERSION`, `OTEL_METRICS_INCLUDE_REPOSITORY` | Metric attributes |
...
Standard exporter variables such as `OTEL_EXPORTER_OTLP_ENDPOINT` also work. `DISABLE_GROWTHBOOK`, `DISABLE_TELEMETRY`, `DO_NOT_TRACK` and `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` each turn off feature-flag fetching, removing Remote Control, the advisor, `/skill-doctor`, `claude import`, auto mode as the default start mode, messaging to other machines and Claude-drafted feedback. Third-party providers and Claude apps gateway sessions skip the fetch too. See [[data-and-privacy]] and [[enterprise-admin]].
...
| `CLAUDE_CODE_DEBUG_LOGS_DIR` | Debug log *file* path; needs debug mode enabled separately |
| `CLAUDE_CODE_DEBUG_LOG_LEVEL` | `verbose`, `debug` (default), `info`, `warn`, `error` |
| `CLAUDE_CONFIG_DIR` | Config directory, default `~/.claude`; e.g. `alias claude-work='CLAUDE_CONFIG_DIR=~/.claude-work claude'` |
| `CLAUDE_CODE_PROJECT_DIR_NAME` | With `CLAUDE_CONFIG_DIR`, choose the `projects/` directory name |
