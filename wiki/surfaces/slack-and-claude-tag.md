...
Two Slack integrations put `@Claude` in your channels. Both run the work in Anthropic's cloud on the same engine as [[claude-code-on-the-web]]; they differ in whose identity and credentials the session uses.
...
**Allow Claude to work in channels with guests** takes **Restrict** (default), **Channel only** or **Allow**, or inherits. Under **Channel only** — also how Slack Connect channels run — Claude has no bundles, repositories, memory, skills or wider instructions, and only the channel's own apply, so treat those as visible to everyone there. In a channel with guests Claude won't search the workspace or read other channels, and guests can't approve a permission request. **Blocked** and **auto-join channel patterns** steer it by name, 50 patterns each with `*` and `?` wildcards; blocked wins.
...
Prerequisites: a plan with Claude Code access, [[claude-code-on-the-web]] enabled with at least one authenticated GitHub repository, and your Slack account linked to your Claude account.
...
3. **Configure the web surface.** Sign in at claude.ai/code with the same account, connect GitHub, and authenticate at least one repository.
...
- **GitHub only** for repositories, and **one pull request per session** for Claude Code in Slack. Without Claude Code on the web enabled, it answers with ordinary chat replies instead of starting a session.
...
| Sessions don't start | Confirm the account is connected in App Home, Claude Code on the web is enabled, and a GitHub repository is connected |
| A Claude Tag channel's sessions fail immediately | The channel points at a personal cloud environment. An Owner recreates it as organization-shared on the **Cloud environments** page, then sets it as the default or on the scope ([[enterprise-admin]]) |
...
Use Slack when the context already lives there, when work should start asynchronously, or when teammates need visibility; use the web for file uploads, real-time interaction and longer tasks. For review automation on the pull request, see [[ci-cd-and-code-review]].
