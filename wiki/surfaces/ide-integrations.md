...
Both work with any paid Claude subscription (Pro, Max, Team, Enterprise) or a Claude Console account. Installing the standalone CLI is covered in [[install-and-setup]].
...
- **Command menu** (`/`):
  ...
  - a Customize section for MCP servers, slash commands, output styles, hooks, permissions and plugins, plus **Memory** (the auto-memory toggles, saved memories and memory folders) and **Instructions** (editing the CLAUDE.md files)
...
- **Indicators**:
  ...
  - an agent count such as **2 agents**, which opens a tree of subagents you can inspect or stop
...
- **Other dialogs**:
  - `/usage` opens Account & usage (claude.ai sign-in only).
...
- **Auto-archive**: sessions inactive for 14 days move to **Archived sessions** unless they are open, unread or grouped. Change this with `archiveInactiveSessions` (`1`, `2`, `7`, `14`, or `0` to turn it off).
...
The extension registers `vscode://anthropic.claude-code/open`. It takes two optional parameters: `prompt` (URL-encoded text that is prefilled but not sent) and `session` (a session ID to resume):
...
3. Run `claude` in the IDE's integrated terminal. From an external terminal, run `/ide` instead. It confirms with a message like `Connected to IntelliJ IDEA.`, and installs the plugin if the running IDE doesn't have it. `claude --ide` connects automatically at startup when exactly one valid IDE is available.
...
- Each activation writes a random token to `~/.claude/ide/<port>.lock`. If `CLAUDE_CONFIG_DIR` is set, the file goes to `$CLAUDE_CONFIG_DIR/ide/` instead.
...
- JetBrains has an **Accept connections from all network interfaces** setting. Turning it on exposes the port to your LAN, and both traffic and token travel in cleartext. For WSL, use mirrored networking instead.
