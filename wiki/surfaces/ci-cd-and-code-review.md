...
Never commit either. For an organization, install the app once at the org level, store the secret as an organization Actions secret, and either copy the workflow into each repository or call one reusable workflow. Prefer an API key, since an OAuth token is tied to one person's subscription.
...
`actions: read` lets Claude read CI results, the checkout step gives it a working copy, and the `if` condition keeps runners from starting on unrelated comments.
...
Two actor checks run before Claude starts, and the run fails if either rejects: the triggering user needs write access (widen with `allowed_non_write_users` plus your own `github_token`), and bots are rejected unless listed in `allowed_bots`, which also covers scheduled runs GitHub attributes to a bot.
...
Self-hosted GitHub works with manual setup only: `/install-github-app` is github.com-only, and the GitHub MCP server doesn't work against GitHub Enterprise Server.
...
Anyone with write, maintain or admin permission can comment `@claude review` to start one review, `@claude review always` to also subscribe the PR to push-triggered reviews, or `@claude review once` (same as the bare command). Manual triggers work on draft PRs, and a fork pull request is reviewed only when someone comments.
...
The **Claude Code Review** check run always concludes neutral, so it never blocks a merge; to gate merges yourself, parse the severity counts from the check run's output text with `gh api` and jq. Findings also appear as annotations in **Files changed**, which survive when GitHub rejects an inline comment on a moved line. Resolve a finding's thread to dismiss it without a code change; replying doesn't dismiss it.

Tune the review with two files: `CLAUDE.md` gives project context, and newly introduced violations of it become nits; `REVIEW.md` at the repository root is review-only instruction text that reaches the finding, verification and ranking agents — use it to redefine severity, cap nit volume, skip paths, add repo-specific checks, raise the verification bar, and shape the summary.

Billing is separate from your plan's included usage, through usage credits: roughly $15-25 per review, scaling with PR size. Reviewing on every push multiplies that. Set a monthly cap at claude.ai/admin-settings/usage; when it's hit, Claude posts one comment saying the review was skipped, and reviews resume next period or when an admin raises the cap. Watch spend at claude.ai/analytics/code-review. A failed or timed-out run never blocks the PR and never retries — comment `@claude review` or click **Re-run**.

## Review locally with /code-review

`/code-review` reviews your branch's commits ahead of upstream plus uncommitted changes, reporting correctness bugs and reuse, simplification and efficiency cleanups; `/review` is an alias. Pass a target instead: a path, PR number, branch, or a range like `main...my-feature`.
...
- On a github.com PR you can post the findings as one plain comment from your own GitHub account; `--no-post` is the default. Claude Code posts nothing itself: it sends the review's session ID to the Anthropic API, which posts the stored findings through your connected GitHub account ([[claude-code-on-the-web]]), so posting needs the same claude.ai sign-in and is unavailable on third-party providers or with `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` set. Claude links you to the comment, to the PR when an earlier post already added it, or says why it failed and leaves the findings in your terminal.
...
From CI or a script, use the subcommand, which blocks until findings arrive and prints them to stdout: `claude ultrareview 1234 --json`, with `--timeout <minutes>` (default 45) and `--post`, which prints the comment link to stderr. It exits 0 when the review completes, 1 on failure or timeout, and 130 on Ctrl-C; progress and the live session URL go to stderr, and a non-interactive run refuses in a repository with no base branch or shared history. Running it is your consent to the charge. In a `claude -p '/code-review ultra'` run Claude Code launches the review and prints a tracking link without waiting, and stops before launching if the run would bill usage credits.
